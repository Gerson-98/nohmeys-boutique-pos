  ...
  return (
    <div className={`card-boutique flex flex-col overflow-hidden transition-opacity ${!isActive ? 'opacity-60' : ''}`}>
      ...
      <div className="flex flex-col flex-1 p-4 gap-2">
        {/* Categoría badge */}
        <span className="inline-flex self-start items-center px-2 py-0.5 bg-blush text-gold rounded-full text-xs font-medium">
          {categoria.icono && <span className="mr-1">{categoria.icono}</span>}
          {categoria.nombre}
        </span>

        <h3 className="font-semibold text-boutique-dark text-sm leading-snug line-clamp-2">{nombre}</h3>

        {descripcion && (
          <p className="text-xs text-boutique-gray-mid line-clamp-2">{descripcion}</p>
        )}
          ...
          <div className="flex flex-wrap gap-1 mt-1">
            {variantes.slice(0, 5).map((v) => (
              <span key={v.id} title={`${v.color ?? ''} ${v.talla ?? ''} — SKU: ${v.sku}`}>
                {v.colorHex ? (
                  <span
                    className="inline-block w-4 h-4 rounded-full border border-gold-light"
                    style={{ backgroundColor: v.colorHex }}
                  />
                ) : (
                  <span className="px-1.5 py-0.5 bg-boutique-gray-soft text-boutique-dark rounded text-xs">
            ...
            {variantes.length > 5 && (
              <span className="text-xs text-boutique-gray-mid">+{variantes.length - 5}</span>
            )}
          </div>
        )}

        <div className="flex items-center justify-between mt-auto pt-2 border-t border-blush">
      ...
      <div className="flex border-t border-blush">
        {puedeEditar && (
          <>
            <button
              onClick={() => onEdit(id)}
              className="flex-1 py-2.5 flex items-center justify-center gap-1 text-xs font-medium text-gold hover:bg-blush-light transition-colors"
            >
              <Pencil size={13} />
              Editar
            </button>
        ...
        <button
          ...
          disabled={toggling}
          className="flex-1 py-2.5 flex items-center justify-center gap-1 text-xs font-medium text-boutique-gray-mid hover:bg-blush-light transition-colors disabled:opacity-50"
        >
          {isActive ? <ToggleRight size={14} className="text-boutique-success" /> : <ToggleLeft size={14} />}
