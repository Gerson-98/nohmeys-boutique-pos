  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="font-playfair text-2xl font-bold text-boutique-dark">Reportes</h1>
          <p className="text-sm text-boutique-gray-mid mt-0.5">Resumen financiero y análisis de ventas</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <input type="date" value={desde} onChange={(e) => setDesde(e.target.value)} className="input-boutique text-sm" />
          <span className="text-boutique-gray-mid text-xs">al</span>
      ...
      {cargando ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: 'Ingresos', value: formatPrecio(resumen?.ingresoTotal ?? 0), icon: <DollarSign size={16} className="text-gold" />, gold: true },
              { label: 'Ganancia bruta', value: formatPrecio(resumen?.gananciaTotal ?? 0), icon: <TrendingUp size={16} className="text-boutique-success" /> },
              { label: 'Margen', value: `${margen}%`, icon: <BarChart2 size={16} className="text-boutique-info" /> },
              { label: 'Ventas', value: (resumen?.cantidadVentas ?? 0).toString(), icon: <ShoppingBag size={16} className="text-boutique-gray-mid" /> },
            ].map((k) => (
          ...
          {serie.length > 0 && (
            <div className="card-boutique p-5">
              <h2 className="font-playfair text-base font-semibold text-boutique-dark mb-4 flex items-center gap-2">
                <TrendingUp size={16} className="text-gold" /> Ingresos diarios
              </h2>
                ...
                {serie.map((s, i) => {
                  ...
                  return (
                    <div key={i} className="flex flex-col items-center gap-1 flex-shrink-0" style={{ minWidth: Math.max(14, Math.floor(400 / serie.length)) }}>
                      <div
                        className="w-full rounded-t-sm bg-gold transition-all hover:bg-gold-light cursor-default group relative"
                        style={{ height: `${Math.max(pct, 2)}%` }}
                        title={`${s.label}: ${formatPrecio(s.ingresos)}`}
                      >
                        {s.ingresos > 0 && (
                          <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-boutique-dark text-white text-[9px] px-1.5 py-0.5 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                            {formatPrecio(s.ingresos)}
                          </div>
                        )}
                      </div>
                      {(i % Math.ceil(serie.length / 10) === 0 || i === serie.length - 1) && (
                        <span className="text-[8px] text-boutique-gray-mid absolute bottom-0 whitespace-nowrap">{s.label}</span>
                      )}
                    </div>
                  );
                })}
          ...
          {topProductos.length > 0 && (
            <div className="card-boutique p-5">
              <h2 className="font-playfair text-base font-semibold text-boutique-dark mb-4 flex items-center gap-2">
                <ShoppingBag size={16} className="text-gold" /> Top 10 productos más vendidos
              </h2>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-0.5">
                          ...
                          <div className="flex items-center gap-3 flex-shrink-0 ml-2">
                            ...
                            <span className="text-xs font-mono font-bold text-gold">{formatPrecio(p.ingresoTotal)}</span>
                          </div>
                        </div>
          ...
          {resumen && (
            <div className="card-boutique p-5">
              <h2 className="font-playfair text-base font-semibold text-boutique-dark mb-3 flex items-center gap-2">
                <DollarSign size={16} className="text-gold" /> Desglose financiero
              </h2>
              <div className="space-y-2">
                {[
                  { label: 'Ingresos brutos', value: resumen.ingresoTotal, cls: 'text-boutique-dark' },
                  { label: 'Costo de ventas', value: -resumen.costoTotal, cls: 'text-boutique-danger' },
                  { label: 'Ganancia bruta', value: resumen.gananciaTotal, cls: 'text-boutique-success font-bold' },
                ].map((r) => (
                  <div key={r.label} className="flex justify-between items-center py-1.5 border-b border-blush last:border-0">
                    <span className="text-sm text-boutique-gray-mid">{r.label}</span>
