      ...
      <DialogContent className="max-w-md rounded-2xl border border-[#F2C4CE] p-0 overflow-hidden">
        <DialogHeader className="px-5 pt-5 pb-3 border-b border-[#F2C4CE] bg-[#FAFAFA]">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-xl bg-[#F8E1E7] flex items-center justify-center overflow-hidden flex-shrink-0">
              {producto.imagenUrl ? (
                ...
                <Package size={20} className="text-[#E8A0B0]" />
              )}
            </div>
            <div className="min-w-0">
              <DialogTitle className="font-semibold text-[#2C2C2C] text-sm leading-snug">
                {producto.nombre}
              </DialogTitle>
              <p className="text-xs text-[#C9A84C] font-mono font-bold mt-0.5">
  ...
  return (
    <button
      ...
      className={`relative py-3 px-4 rounded-xl border-2 text-sm font-medium transition-all
        ${agotado
          ? 'border-[#E2E8F0] text-[#9E9E9E] bg-[#F5F5F5] cursor-not-allowed line-through'
          : 'border-[#E8D5A3] text-[#2C2C2C] hover:border-[#C9A84C] hover:bg-[#F8E1E7] active:scale-95'
        }`}
    >
      {label}
      {agotado && (
        <span className="absolute -top-1.5 -right-1.5 text-[8px] bg-[#E57373] text-white rounded-full px-1 font-bold">
          0
        </span>
      )}
      {!agotado && (
        <span className="block text-xs text-[#9E9E9E] font-normal mt-0.5">
...
function SeccionVariantes({
  ...
  return (
    <div>
      <p className="text-xs font-medium text-[#9E9E9E] uppercase tracking-wide mb-2">{titulo}</p>
            ...
            return (
              <button
                ...
                className={`w-10 h-10 rounded-full border-2 transition-all relative
                  ${agotado ? 'opacity-40 cursor-not-allowed' : 'hover:scale-110 border-[#E8D5A3] hover:border-[#C9A84C]'}`}
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-xs">
        <thead>
          <tr>
            <th className="text-left p-1.5 text-[#9E9E9E] font-medium">Talla \ Color</th>
            {colores.map((c) => (
              <th key={c} className="p-1.5 text-[#2C2C2C] font-medium text-center">{c}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {tallas.map((talla) => (
            <tr key={talla}>
              <td className="p-1.5 font-medium text-[#2C2C2C]">{talla}</td>
              {colores.map((color) => {
                const v = variantes.find((x) => x.talla === talla && x.color === color);
                if (!v) return <td key={color} className="p-1.5 text-center text-[#9E9E9E]">—</td>;
                const agotado = v.stockActual === 0;
                return (
                  <td key={color} className="p-1.5 text-center">
                    <button
                      ...
                      className={`w-12 py-1.5 rounded-lg text-xs font-medium border transition-all
                        ${agotado
                          ? 'bg-[#F5F5F5] border-[#E2E8F0] text-[#9E9E9E] cursor-not-allowed'
                          : 'bg-white border-[#E8D5A3] hover:border-[#C9A84C] hover:bg-[#F8E1E7] active:scale-95'
                        }`}
                    >
